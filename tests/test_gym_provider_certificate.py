...
def test_issue_certificate(provider_certificate_contract, owner, provider):
    ...
    tx = provider_certificate_contract.issueCertificate(provider, 2, sender=owner)
    tx.wait()
...
def test_validate_certificate(provider_certificate_contract, owner, provider):
    provider = provider.address if type(provider) != str else provider
    stake_contract_address = provider_certificate_contract.stakingContract()
    stake_contract = project.Stake.at(stake_contract_address)
    stake_contract.stake(1000, True, sender=provider).wait()
...
def test_revoke_certificate(provider_certificate_contract, owner, provider):
    provider = provider.address if type(provider) != str else provider
    stake_contract_address = provider_certificate_contract.stakingContract()
    stake_contract = project.Stake.at(stake_contract_address)
    stake_contract.stake(1000, True, sender=provider).wait()
