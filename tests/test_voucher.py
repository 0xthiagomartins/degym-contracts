...
def test_create_voucher(voucher_contract, owner, consumer):
    consumer = consumer.address if type(consumer) != str else consumer
    initial_balance = voucher_contract.balanceOf(consumer)
    tx = voucher_contract.createVoucher(consumer, 1, 30, "UTC", sender=owner)
    tx.wait()
    assert voucher_contract.balanceOf(consumer) == initial_balance + 1
    assert voucher_contract.getVoucherDetails(0)["tier"] == 1


def test_upgrade_voucher(voucher_contract, owner, consumer):
    consumer = consumer.address if type(consumer) != str else consumer
    voucher_contract.createVoucher(consumer, 1, 30, "UTC", sender=owner).wait()
...
def test_renew_voucher(voucher_contract, owner, consumer):
    consumer = consumer.address if type(consumer) != str else consumer
    voucher_contract.createVoucher(consumer, 1, 30, "UTC", sender=owner).wait()
...
def test_downgrade_voucher(voucher_contract, owner, consumer):
    consumer = consumer.address if type(consumer) != str else consumer
